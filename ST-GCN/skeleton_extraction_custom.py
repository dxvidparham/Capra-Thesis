...
from opendr.perception.pose_estimation.lightweight_open_pose.filtered_pose import (
    FilteredPose,
)
...
def infer3Dpose(pose_estimator, img, upsample_ratio=4, track=True, smooth=True):
    ...
    :rtype img: engine.data.Image id object
...
def get_file_ids(file, query):
    """Return the respective camera, subject or action id for a given file."""
    index = file.find(query)
    return int(file[index + 1 : index + 3])
...
def data_gen(args, pose_estimator, out_path, benchmark, part):
    ...
    for filename in os.listdir(args.videos_path):
        subject_id = get_file_ids(filename, "P")
        camera_id = get_file_ids(filename, "C")
        action_id = get_file_ids(filename, "A")
        ...
        if issample:
            ...
            sample_labels.append(action_id)

    with open(f"{out_path}/{part}_labels.pkl", "wb") as f:
    ...
    for i, s in enumerate(tqdm(sample_names)):
        video_path = os.path.join(args.videos_path, f"{s}.avi")
    ...
    np.save(f"{out_path}/{part}_joints.npy", skeleton_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--videos_path",
        type=str,
        default="data/Dataset/unified",
        help="path to video files",
    )
    parser.add_argument(
        "--num_channels", help="number of channels for each keypoint", default=2
    )
    parser.add_argument("--out_folder", default="data/2D_skeletons")
    ...
    # Optimization
    if onnx:
        print("[INFO] ONNX model found, model will be optimized.")
